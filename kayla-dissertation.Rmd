...
```{r PANAS Reliability, echo=FALSE, include=FALSE}
...
## 3. Independent T-Tests for H1

```{r indep t-tests for residuals, echo=FALSE}
...
## 4. Assumptions for H1

```{r h1 assumption, echo=FALSE, warning=FALSE}
...
```{r make h2data, echo=FALSE, include=FALSE}
...
```{r h2box data, echo=FALSE, include=FALSE}
h2_box <- h2_data %>%
...
## 1. Boxplot for Hypothesis 3
```{r FIG2 h2box, echo=FALSE}
h2_box %>%
...
## 4. Assumptions for h4
```{r h2 assumption, echo=FALSE}
...
h3_box %>%
  ...
  labs(x = "Time",
       y = "SASB Affiliation",
       title = "Figure 3. Boxplots of SASB Affiliation by Interpersonal Surfaces \nand Group Pre and Post Substance Use",
       caption = "Note. SOST = Significant Other to Subject Transitive; SOSI = Significant Other to Subject Intransitive; \nSSOT = Subject to Significant Other Transitive; SSOI = Subject to Significant Other Intransitive") +
  scale_color_manual(values=c("#AAAAAA", "#60B2FF")) +
  theme(plot.caption = element_text(hjust = 0))
```
...
## 2. Group Differences using Independent T-Tests (H3)
```{r t for h3, echo=FALSE}
...
## 1. Summary Statistics for H4

Mean and Standard Deviations by Group for PANAS Pre and Post
```{r, echo=FALSE}
panas_data %>%
  group_by(group) %>%
...
Minimum and Maxiumum by Group for PANAS Pre and Post
```{r, echo=FALSE}
panas_data %>%
  group_by(group) %>%
...
## 2. Boxplot for PANAS Negative Affect Change by Group
```{r fig4, echo=FALSE}
panas_data %>% 
  gather(panas_negative, score, panas2_1, panas2_2, -client_number) %>%
...
T-test for H4 Using Substance Use Group Only
...
T-test for H4 Using Non-Substance Use Group Only
...
## 1. Boxplot for PANAS Positive Affect Change by Group
```{r FIG5, echo=FALSE}
panas_data %>% 
  gather(panas_positive, score, panas1_1, panas1_2, -client_number) %>%
...
```{r h61 box data, echo=FALSE, include=FALSE}
h3_box <- coefficents_long %>%
  filter(surface == "surf_1") %>%
  dplyr::select(-group) %>%
  gather(time, value, -group_type,-client_number, -surface) %>%
