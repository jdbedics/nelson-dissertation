---
title: "Nelson Data Analysis for Doctoral Dissertation"
author: "Kayla Nelson"
date: "4/22/2019"
output:
  html_document:
...
Here is the key: surf_1	INTAFF; surf_2	INTAUT; surf_3 SOSTAFF; surf_4 SOSTAUT; surf_5 SOSIAFF; surf_6 SOSIAUT; surf_7 SSOTAFF; surf_8 SSOTAUT; surf_9 SSOIAFF;surf_10 SSOIAUT
...
H1: Participants seeking treatment for substance abuse will report greater hoped-for affiliation in a reported significant-other relationship following use of their substance of choice compared to participants not seeking treatment for substance abuse. (Do not analyze drug of choice as stated in data analytic plan)

1. Boxplot for Affiliation Scores by Group at Pre- and Post. 
2. Descriptive analyses using pre and post paired t-tests.
3. Differences between groups (substance versus non-substance) in residual change for affiliation
4. Visualization of T-test statistics.


### 1. Boxplot for Affiliation in Interpersonal Relationships

Boxplot for SASB Affiliation Scores by Group and Pre- and Post-Substance Use (N=31)
```{r wrangle data for h1 boxplot, echo=FALSE}
...
h1_box %>%
  ...
  labs(x = "Time",
       y = "SASB Affiliation",
       title = "Boxplots of SASB Affiliation by Interpersonal Surfaces \nand Group Pre and Post Substance Use",
...
### 2. Paired Sample T-tests for Difference Scores
...
Straightforward Paired T-Tests for SASB Surfaces 3, 5, 7, 9
```{r}
# Here's the formula
#result <- t.test(before, after, paired = TRUE)
# have to use coefficient data in wide format
...
coefficients %>%
  ...
  summarize(statistic = t.test(unlist(surf_9_1), unlist(surf_9_2), paired = TRUE)$statistic,
            ...
            Df = t.test(unlist(surf_9_1), unlist(surf_9_2), paired = TRUE)$df)
...
### 3. Group Differences using Independent T-Tests (H1)
...
H2: Adolescents who reported depressants (alcohol, marijuana, narcotics) as their drug of choice will rate their significant others as more sulking and resentful prior to using compared to adolescents who do not report depressants as their drug of choice. 
```{r make h2data}
...
h2_box %>%
  ...
  labs(x = "Time",
       y = "Sulk and Scurry",
       title = "Boxplots of SASB Cluster 6 Significant Other Sulks and Scurrys \nand Drug Type Pre and Post Substance Use",
       caption = "") +
  scale_color_manual(values=c("#AAAAAA", "#60B2FF")) +
  theme(plot.caption = element_text(hjust = 0)) +
  geom_jitter()
...
#t.test(h2_data_na$sosicl6a ~ h2_data_na$doc_fa) # after


####non-parametric for imbalanced data
#wilcox.test(h2_data_na$sosicl6a ~ h2_data_na$doc_fa)
...
1. Visualize introject affiliation differences between groups at their reported 'best' and at their reported 'worst'. Boxplots.
2. Test differences in introject affiliation in both groups using paired t-tests.
3. Calculate residual change scores for introject (worst predicted by best) and compare differences in residuals by group (substance or no-substance).

### 1. Boxplot for Affiliation in Introject (H3)
```{r}
...
Paired T-test for pre and post difference scores in affiliation for all surfaces and all groups - not comparing groups - and for interpersonal surfaces.
```{r introject surface tests, echo=FALSE}
coefficents_long %>%
  filter(surface == "surf_1") %>%
  group_by(surface, group) %>%
  do(tidy(t.test(.$pre, .$post, paired=TRUE)))  
...
### 3. Group Differences using Independent T-Tests (H3)
```{r}
...
panas_data %>% 
  ...
  labs(x="Negative Affect",
       title = "Figure #. Change in Negative Affect (PANAS) Prior and After Substance Use") +
  theme(legend.position = "none")
```
...
```{r overall difference in negative affect, echo=FALSE}
...
```{r substance use people only, echo=false}
su <- panas_data %>%
  filter(group_type == "Substance Use")

t.test(su$panas2_2, su$panas2_1, paired = TRUE, alternative = "two.sided")
# substance use people had a significant reduction (again, just change isn't differen from nongroup)
```
Substance use participants showed a significant decrease in negative following substance use (t(19)=-2.16, p=0.04)

```{r non-substance use people only, echo=false}
nsu <- panas_data %>%
...
```
Non-substance use participants did not show a significant decrease in negative following substance use (t(10)=-0.11, p=0.91)
...
#### PANAS Positive Affect Change by Group
```{r}
panas_data %>% 
  gather(panas_positive, score, panas1_1, panas1_2, -client_number) %>%
  ...
  labs(x="Positive",
       title = "Figure #. Change in Positive Affect (PANAS) Prior and After Substance Use") +
  theme(legend.position = "none")
```
...
```{r overall difference in positive affect, echo=FALSE}
t.test(panas_data$panas1_2, panas_data$panas1_1, paired = TRUE, alternative = "two.sided")
```
The difference in negative affect change following substance use was non-significant between substance and non-substance use participants (t(30)=-1.02, p=0.32)


```{r substance use people only positive, echo=false}
su <- panas_data %>%
  filter(group_type == "Substance Use")

t.test(su$panas1_2, su$panas1_1, paired = TRUE, alternative = "two.sided")
```
Substance use participants showed a significant decrease in negative following substance use (t(19)=1.03, p=0.32)

```{r non-substance use people only positive, echo=false}
nsu <- panas_data %>%
  filter(group_type == "Non-Substance")

t.test(nsu$panas1_2, nsu$panas1_1, paired = TRUE, alternative = "two.sided")
```
Non-substance use participants did not show a significant decrease in negative following substance use (t(10)=-0.33, p=0.75)
...
### H61: The more parental monitoring the more decrease of negative affect following substance use
```{r h6-wrangling}
...
h6_data %>%
  ...
  labs(x = "Residual Change in PANAS",
       ...
       title = "Figure #. Scatter plot of parental monitor \n by change in positive and negative PANAS",
       caption = "") +
  scale_color_manual(values=c("#AAAAAA", "#60B2FF")) +
  theme(plot.caption = element_text(hjust = 0))
```

```{r monitoring correlations}
...
### H62: The more peer acceptance the greater increase in positive affect substance use
```{r acceptance graph}
h6_data %>%
  gather(panas_type, panas_value, -pmtot, -psmtot) %>%
  mutate(panas_f = factor(panas_type, labels = c("Positive", "Negative"))) %>%
  ...
  labs(x = "Residual Change in PANAS",
       ...
       title = "Figure #. Scatter plot of peer support \nby change in positive and negative PANAS",
       caption = "") +
  scale_color_manual(values=c("#AAAAAA", "#60B2FF")) +
  theme(plot.caption = element_text(hjust = 0))
```
...
### H63: The more more peer acceptance the more affiliative a significant other is seen acting toward the participant following substance use. 
```{r acceptance-affiliation}
h63_data <- h6_data %>%
