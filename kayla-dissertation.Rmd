...
#create a dataframe that excluded participants that never used substances
kayla_no_non <- kayla_raw %>%
  filter(!client_number == "1819" & !client_number == "1820")
```

```{r create processed data}
#replace "kayla_raw" with "kayla_no_non" to examine differences between results with and without clients that never used substances.
...
```{r PANAS Reliability, include=FALSE, eval=FALSE}
...
```{r PM Reliability, include=FALSE, eval=FALSE}
pm.df <- kayla_processed[ , c("pm_1", "pm_2", "pm_3")] 
CronbachAlpha(pm.df) # .78
```


```{r PSM Reliability, include=FALSE, eval=FALSE}
psm.df <- kayla_processed[ , c("psm_1", "psm_2", "psm_3")] 
CronbachAlpha(psm.df) # .74
```
...
h1_box %>%
  ...
  labs(x = "Time",
       y = "SASB Affiliation",
       title = "Figure 2. Boxplots of SASB Affiliation by Interpersonal Surfaces \nand Group Pre and Post Substance Use",
...
h2_box %>%
  ...
  labs(x = "Time",
       y = "Sulk and Scurry",
       title = "Figure 3. Boxplots of SASB Cluster 6 Significant Other Sulks and Scurrys \nand Drug Type Pre and Post Substance Use",
       caption = "") +
  scale_color_manual(values=c("#AAAAAA", "#60B2FF")) +
  theme(plot.caption = element_text(hjust = 0))
```
...
h2_data_na %>%
  summarise(h2_w = wilcox.test(sosicl6p ~ doc_fa)$statistic,
            h2_sig = wilcox.test(sosicl6p ~ doc_fa)$p.value)
```
...
h3_box %>%
  ...
  labs(x = "Time",
       ...
       title = "Figure 4. Boxplot of SASB Introject Affiliation by Treatment Group  and Pre- and Post-Substance Use",
       caption = "") +
  scale_color_manual(values=c("#AAAAAA", "#60B2FF")) +
  theme(plot.caption = element_text(hjust = 0))
```
...
panas_data %>% 
  ...
  labs(x="Negative Affect",
       title = "Figure 5. Change in Negative Affect (PANAS) Prior and After Substance Use") +
...
panas_data %>% 
  ...
  labs(x="Positive",
       title = "Figure 6. Change in Positive Affect (PANAS) Prior and After Substance Use") +
  theme(legend.position = "none")
```
...
h6_data %>%
  ...
  labs(x = "Residual Change in PANAS",
       ...
       title = "Figure 7. Scatter plot of parental monitor by change in positive and negative PANAS (95% CI)",
       caption = "") +
  scale_color_manual(values=c("#AAAAAA", "#60B2FF")) +
  theme(plot.caption = element_text(hjust = 0))
```
...
```{r h61-correlations}
h6_data %>%
  summarise(neg_pmtot_cor = cor.test(panas2_res, pmtot)$estimate,
            neg_pmtot_sig = cor.test(panas2_res, pmtot)$p.value,
            pos_pmtot_cor = cor.test(panas1_res, pmtot)$estimate,
            pos_pmtot_sig = cor.test(panas1_res, pmtot)$p.value)
```
...
h6_data %>%
  ...
  labs(x = "Residual Change in PANAS",
       ...
       title = "Figure 8. Scatter plot of peer support by change in positive and negative PANAS (95% CI)",
       caption = "") +
  scale_color_manual(values=c("#AAAAAA", "#60B2FF")) +
  theme(plot.caption = element_text(hjust = 0))
```
...
```{r h62-correlations}
h6_data %>%
  summarise(neg_psmtot_cor = cor.test(panas2_res, psmtot)$estimate,
            neg_psmtot_sig = cor.test(panas2_res, psmtot)$p.value,
            pos_psmtot_cor = cor.test(panas1_res, psmtot)$estimate,
            pos_psmtot_sig = cor.test(panas1_res, psmtot)$p.value)
```
...
```{r}
h63_data %>%
  summarise(neg_pmtot_cor = cor.test(surf_5, psmtot)$estimate,
            neg_pmtot_sig = cor.test(surf_5, psmtot)$p.value)
```
